...
type pid = string
type traceT = list (pid & pid & bytes)

(* for some reason this `unfold` makes everything 25% faster *)
unfold type store = id & tape #byte & traceT
...
effect Wald (a:Type) =
  WALD a (fun initial_tape post -> forall (x:option a & (id & traceT)). post x)


(** If not past the end of the tape, read a value and advance pointer *)
reifiable val sample: unit -> WALD elem (fun (next,t,tr) p ->
  if incrementable next then p (Some (index t next), (incr next, tr))
  else p (None, (next, tr)))
let sample () =
  let next, t, tr = WALD?.get () in
  if incrementable next then
    begin
    WALD?.put (incr next, tr);  // roughly RAND.next += 1
...
reifiable val sample_n: n:nat -> WALD (r: Seq.seq elem {Seq.length r == n}) (fun (next,t,tr) p ->
...
let sample_n n =
  ...
  if incrementable_by_n next n then
    ...
    begin
    WALD?.put (largest_id, tr);
    WALD?.raise (Seq.seq elem) ()
    end

reifiable val trace (from to: pid) (msg: bytes) : WALD unit (fun (next,_,tr) p ->
  p (Some (), (next, (from,to,msg) :: tr)))
let trace from to msg =
  let next, _ , tr = WALD?.get () in
  let tr' = (from, to, msg) :: tr in
  WALD?.put (next, tr')

let extract_trace (#a: Type) (r: option a & (id & traceT)) : traceT = let _, (_, tr) = r in tr

// TODO(klinvill): it seems this lemma is needed to prove that rand_bytes returns a value. Is there a more elegant way to do this?
...
// TODO(klinvill): it seems this lemma is needed to prove that rand_bytes advances the tape pointer by a given amount. Is there a more elegant way to do this?
...
assume val sum: f:(tape #byte -> nat) -> nat
...
(* TODO(klinvill): The Wald effect doesn't have much information on traces. I
  believe this is leading to pr_leq to fail to prove unless mass is unfolded.
  The Wald effect should ideally have a more informative type such that
